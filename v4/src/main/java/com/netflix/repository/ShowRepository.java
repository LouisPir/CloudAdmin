...
// V4-fix – exact-match filters + column projection + local list cache (Redis variant)
//   Aggregation caching stays in Redis (via CacheService).
//   List caching is JVM-local because the Redis round-trip (~10ms)
//   would negate the benefit of caching a 14ms query.
@Repository
public class ShowRepository {
    ...
    //    Intentionally NOT in Redis — a ~10ms Redis hop on a ~14ms query
    //    would halve the benefit. Local cache gives sub-2ms responses.
